...
class SessionManagerClientImpl : public SessionManagerClient {
 public:
  ...
  void ShellStateCallback(absl::optional<jemaos::ash::ShellState> state) {
    if (state) {
      VLOG(1) << "Device wipe call JemaOSShellClient, callback, state code:"
              << state->code;
      ...
      if (!jemaos::switches::IsJemaCustomEnabled()) {
        // if jemaos_shell_client state null(error calling jemaos shell daemon),
        // and jema-disable-custom flags in chrome_dev.conf,
  ...
  void StartDeviceWipe() override {
    ...
    jemaos::ash::JemaOSShellClient::Get()->SyncExec(
      "/usr/sbin/clobber",
