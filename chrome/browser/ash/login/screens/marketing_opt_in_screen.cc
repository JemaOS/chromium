...
bool MarketingOptInScreen::MaybeSkip(WizardContext& context) {
  ...
  if (chrome_user_manager_util::IsPublicSessionOrEphemeralLogin() ||
      IsCurrentUserManaged() || (!context.is_branded_build && !jemaos::switches::IsJemaCustomEnabled())) {
    exit_callback_.Run(Result::NOT_APPLICABLE);
    return true;
  }
...
void MarketingOptInScreen::ShowImpl() {
  ...
  const bool legal_footer_visible = false;
  const bool opt_in_by_default_state = email_opt_in_visible_;
  ...
  if (view_) {
    view_->Show(/*opt_in_visible=*/email_opt_in_visible_,
                /*opt_in_default_state=*/opt_in_by_default_state,
...
void MarketingOptInScreen::OnUserAction(const base::Value::List& args) {
  ...
  if (action_id == kUserActionGetStarted) {
    ...
    OnGetStarted(chromebook_email_opt_in, jemaos_improvement_plan_opt_in);
    return;
  }
...
void MarketingOptInScreen::OnGetStarted(bool chromebook_email_opt_in, bool jemaos_improvement_plan_opt_in) {
  if (is_hidden())
    return;
  DCHECK(initialized_);
  ...
  if (email_opt_in_visible_) {
    ...
    profile->GetPrefs()->SetBoolean(jemaos::prefs::kJemaOSImprovementPlanEnabled,
                                    jemaos_improvement_plan_opt_in);
...
void MarketingOptInScreen::Initialize() {
  ...
  email_opt_in_visible_ = ShouldShowOptionToSubscribe();
