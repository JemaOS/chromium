...
// static
bool JemaAssistantBubble::ReadyToInit() {
...
bool JemaAssistantBubble::InitWebView(JemaAssistantView* owner) {
  owner_ = owner;
  return OpenUrl(GURL(kJemaAssistantExtensionUrl));
}

void JemaAssistantBubble::RemoveWebView() {
  if (web_view_ptr_) {
...
bool JemaAssistantBubble::OpenUrl(const GURL& url) {
