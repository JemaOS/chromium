// os-settings-jemaos-license-info
...
import {getTemplate} from './jemaos_license_info.html.js';
...
class LicenseInfoClient {
  ...
  async GetLicenseID() {
    ...
    const ret = await this.shellClient_.SafeExecForResult('/usr/share/jemaos_shell/license-utils.sh id');
    return ret;
  }
...
async GetLicenseExpireDate() {
    ...
    const text = await this.shellClient_.SafeExecForResult('/usr/share/jemaos_shell/license-utils.sh read');
    if (!text) return '';
    let data;
    try {
...
const JemaSettingsLicenseInfoPageElementBase = I18nMixin(PolymerElement);

/** @polymer */
class JemaSettingsLicenseInfoPageElement extends JemaSettingsLicenseInfoPageElementBase {
  static get is() {
    return 'os-settings-jemaos-license-info';
  }
  static get template() {
    return getTemplate();
  }
  ...
  private JEMAOS_LICENSE_INFO_URL: string;
  ...
  private jemaosSettingsLicenseUrl: string;
  ...
  constructor() {
    super();
    this.jemaosSettingsLicenseUrl = loadTimeData.getString('jemaosSettingsLicenseUrl');
    const licenseUrl = new URL(this.jemaosSettingsLicenseUrl);
  ...
  async load_() {
    ...
    if (this.licenseID) {
      this.webview_.src = this.appendExtraParamsForUrl_(`${this.jemaosSettingsLicenseUrl}&licenseId=${this.licenseID}&serialNumber=${this.serialNumber}`);
      this.isWebviewUrlSet = true;
    } else {
      this.isLoading = false;
      this.getIdError = true;
      this.errorMessage = this.i18n('jemaosSettingsLicenseErrorReadMachineId');
    }
  }
  ...
  setupTunnel() {
    ...
    if (this.webview_.contentWindow) {
      this.webview_.contentWindow.postMessage({
        type: 'jemaos-license-info-channel-message',
        channelId: this.channelId,
        payload: {
          name: 'jemaos-license-info-internal-reply-message',
          extra: {
            email,
  ...
  onMessage_(message: MessageEvent) {
    ...
    if (type === 'jemaos-license-info-channel-message' && channelId === this.channelId) {
  ...
  appendExtraParamsForUrl_(url: string) {
    ...
    const board = loadTimeData.getString('jemaosBoardName');
    return `${url}&board=${board}&hl=${lang}`;
  }

  setLoadingState_(loadingText: string) {
    this.isLoading = true;
    this.loadingText = loadingText || this.i18n('jemaosSettingsLicenseStateLoading');
  }
...
customElements.define(
    JemaSettingsLicenseInfoPageElement.is, JemaSettingsLicenseInfoPageElement);
