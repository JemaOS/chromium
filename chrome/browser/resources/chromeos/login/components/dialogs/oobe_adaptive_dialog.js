...
export class OobeAdaptiveDialog extends PolymerElement {
  ...
  iconJemaTweak_() {
    // replace hardcode google-blue-600 #1A73E8 with jemaos-pink-600
    // these icons are defined in ../oobe_icons.html and used as `oobe-32:xxx`
    const body = document.querySelector('body');
    if (!body || !body.classList.contains('jemaos')) return;
    [
      'slot[name="icon"]',
      'slot[name="subtitle"]',
      'slot[name="content"]',
    ].forEach((selector) => {
      this.jemaModifyIconColorOfSlot(selector);
    })
  }
 
  jemaModifyIconColorOfSlot(slotSelector) {
    ...
    if (slot && (typeof slot.assignedNodes === 'function')) {
      slot.assignedNodes().forEach((node) => {
        if (node.tagName && targetTags.includes(node.tagName.toLowerCase())) {
          this.jemaModifyIconColor(node);
          return;
        }
        const icons = node.querySelectorAll(targetTags.join(','));
        icons.forEach((icon) => {
          this.jemaModifyIconColor(icon);
        });
      });
    }
  }
 
  jemaModifyIconColor(icon) {
    if (!icon.shadowRoot) {
      return;
    }
    const selectors = [
    ...
    icon.shadowRoot.querySelectorAll(selectors).forEach((ele) => {
      ele.setAttribute('fill', 'var(--jemaos-pink-600)');
    });
  }
