  ...
  switch (enrollment_config_.auth_mechanism) {
    ...
    case EnrollmentConfig::AUTH_MECHANISM_BEST_AVAILABLE:
      ...
      next_auth_ = enrollment_config_.should_enroll_interactively()
                       ...
                       : AUTH_ATTESTATION;
      break;
    case EnrollmentConfig::AUTH_MECHANISM_JEMA:
      current_auth_ = AUTH_JEMA;
      next_auth_ = AUTH_JEMA;
      break;
    case EnrollmentConfig::AUTH_MECHANISM_JEMA_BEST_AVAILABLE:
      current_auth_ = AUTH_JEMA;
      next_auth_ = enrollment_config_.should_enroll_interactively()
                       ? AUTH_OAUTH
                       : AUTH_JEMA;
...
void EnrollmentScreen::SetConfig() {
  ...
  } else if (current_auth_ == AUTH_OAUTH && config_.is_mode_jema()) {
    config_.mode = config_.is_forced()
        ? policy::EnrollmentConfig::MODE_LOCAL_FORCED
        : policy::EnrollmentConfig::MODE_MANUAL;
  } else if (current_auth_ == AUTH_JEMA && !config_.is_mode_jema()) {
    config_.mode = policy::EnrollmentConfig::MODE_JEMA_LOCAL_FORCED;
  }
...
bool EnrollmentScreen::AdvanceToNextAuth() {
  if (current_auth_ != next_auth_ && (current_auth_ == AUTH_ATTESTATION || current_auth_ == AUTH_JEMA)) {
...
void EnrollmentScreen::UpdateFlowType() {
  ...
  if (cfm) {
    ...
    if (!features::IsKioskEnrollmentInOobeEnabled() || config_.is_mode_jema()) {
...
void EnrollmentScreen::AuthenticateUsingJema() {
  ...
  if (view_)
    view_->Show();
  CreateEnrollmentHelper();
  std::string jemaToken;
  if (base::ReadFileToString(base::FilePath(jemaos::constants::kJemaEnrollmentTokenFilePath), &jemaToken)) {
    jemaToken = base::CollapseWhitespaceASCII(jemaToken, true);
  }
  enrollment_helper_->EnrollUsingJemaToken(jemaToken);
}
...
void EnrollmentScreen::SetupAndShowOfflineMessage(
  ...
  if (LoginDisplayHost::default_host()->GetOobeUI()->current_screen() !=
      ErrorScreenView::kScreenId) {
    error_screen_->AllowJemaLocalSignin(
        !WizardController::IsJemaZeroTouchEnrollFlow());
