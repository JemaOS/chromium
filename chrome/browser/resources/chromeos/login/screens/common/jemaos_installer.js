...
"blockdevices": [
  ...
  async Install({ diskPath }) {
    ...
    return this.isARM().then((ret) => {
      ...
      if (JemaOSInstaller.SkipRemovableBoards.indexOf(board) !== -1) {
  ...
  async clearDualbootLog() {
    try {
      await this.execForResult(JemaOSInstaller.UtilWrapperCMD('clear_log'));
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async GetDualBootInstallLog() {
    const ret = await this.getFileContent('/tmp/jemaos_dualboot.log');
    return ret;
  }
  ...
  DualbootInstall({ partPath, efiPath, installrEFI = true, installBoot = false }) {
    ...
    function dispatchError(desc, err) {
      if (self.forceClosed) return;
      self.dispatchEvent(JemaOSInstaller.Events.OnError, { detail: { error: err, desc } });
    }
    ...
    const process = [
      ...
      {
        command: JemaOSInstaller.InstallKernelCMD(partPath),
        enable: true,
      ...
      {
        command: JemaOSInstaller.InstallImageCMD(partPath),
        enable: true,
      ...
      {
        command: JemaOSInstaller.InstallLoaderCMD(efiPath),
        enable: true,
      ...
      {
        command: JemaOSInstaller.InstallrEFIndCMD(efiPath),
        enable: installrEFI,
...
const availEFIPart = dev => {
  const okSize = JemaOSInstaller.SizeInBytes(dev.size) > 0;
  const notRm = dev.rm !== '1';
  const notRo = dev.ro !== '1';
  const isEfi = dev.parttype === JemaOSInstaller.EFI_TYPE;
  return okSize && notRm && notRo && isEfi;
}

const candidateOSPart = dev => {
  return availOSPart(dev) && JemaOSInstaller.SizeInBytes(dev.size) > recommandedSize && !(dev.mountpoint && dev.mountpoint.startsWith('/tmp/jemaos_dualboot_'));
}
...
const DiskToSelectOption = disk => {
  let title = JemaOSInstaller.GetDevPath(disk.name);
  if (disk.size) {
    title = `${JemaOSInstaller.GetDevPath(disk.name)} (${disk.size})`;
  }
  const value = disk.name;
  return { title, value };
};

const JemaOSInstallerScreenElementBase = mixinBehaviors(
  [
    OobeI18nBehavior,
 ...
 * @polymer
 */
class JemaOSInstallerScreen extends JemaOSInstallerScreenElementBase {
  static get is() {
    return 'oobe-jemaos-installer';
  }
  ...
  ready() {
    ...
    this.installer_ = new JemaOSInstaller(this);

    this.addEventListener(JemaOSInstaller.Events.OnProgress, this.handleInstallProgess_.bind(this));
    this.addEventListener(JemaOSInstaller.Events.OnSuccess, this.handleInstallSuccess_.bind(this));
    this.addEventListener(JemaOSInstaller.Events.OnError, this.handleInstallError_.bind(this));
  ...
  setupMultiBootOptionsSelect_() {
    this.multiBootOptions_ = [{
      title: this.i18n('jemaosInstallerInstallREFIndTitle'),
      value: MultiBootOptions.REFIND,
    }, {
      title: this.i18n('jemaosInstallerInstallUefiBootTitle'),
      value: MultiBootOptions.UEFI,
    }, {
      title: this.i18n('jemaosInstallerMultiBootConfigureByYourselfTitle'),
  ...
  async isRootBSetup(rootPart, list) {
    ...
    const rootb = list.find(p => JemaOSInstaller.GetDevPath(p.name) === rootbName && p.type === 'part');
    const roota = list.find(p => JemaOSInstaller.GetDevPath(p.name) === rootPart && p.type === 'part');
    if (!rootb || !roota) {
      return true;
    }
    const sizea = JemaOSInstaller.SizeInBytes(roota.size);
    const sizeb = JemaOSInstaller.SizeInBytes(rootb.size);
    return sizea <= sizeb;
  }
  ...
  async waitForShellClient_() {
    ...
    const concernRootB = (JemaOSInstaller.SpecialCaseBoards.indexOf(board) !== -1);
    const forceShow = this.IS_DEBUG || (JemaOSInstaller.ForceEnableForBoards.indexOf(board) !== -1);
    ...
    this.fire('show-jemaos-installer-button', { visible });
  ...
  show() {
    console.log('jemaos installer show');
  }
  ...
  handleInstallSuccess_() {
    ...
    this.installOutput_ = '';
    this.progressMessage_ = this.i18n('jemaosInstallerInstallSuccessMessage');
  }
  ...
  isRootDev_(disk) {
    ...
    return this.rootDev_.startsWith(JemaOSInstaller.GetDevPath(disk.name));
  }
  ...
  async prepareConfigForNormalInstall_() {
    ...
    const list = this.fullDiskList_.filter(d => {
      ...
      const enoughDiskSize = (!d.size) || (d.size && JemaOSInstaller.SizeInBytes(d.size) > minimumSize);
      return available && enoughDiskSize;
    });
    if (!list.length) {
      this.normalConfigMessage_ = this.i18n('jemaosInstallerNormalConfigNoAvailableDisk');
      return;
    }
  ...
  async prepareConfigForDualbootInstall_() {
    ...
    for (let i = 0; i < availableDiskList.length; i++) {
      ...
      osPartList.push({ optionGroupName: JemaOSInstaller.GetDevPath(disk.name) });
      efiPartList.push({ optionGroupName: JemaOSInstaller.GetDevPath(disk.name) });
  ...
  handleDualbootConfigOSPartChange_(partName) {
    ...
    if (dev.mountpoint && dev.mountpoint.startsWith('/tmp/jemaos_dualboot_')) {
      this.dualbootConfigSelectOSPartMessage_ = this.i18n('jemaosInstallerDualbootSelectedOSPartAlreadyInstalled');
    } else if (JemaOSInstaller.SizeInBytes(dev.size) < recommandedSize) {
      this.dualbootConfigSelectOSPartMessage_ = this.i18n('jemaosInstallerDualbootSelectedOSPartSizeSmaller');
    } else {
      this.dualbootConfigSelectOSPartMessage_ = '';
    }
  }
  ...
  onMultiBootOptionSelectedChanged_() {
    switch (this.multiBootOptionSelected_) {
      case MultiBootOptions.REFIND:
        this.multiBootOptionDesc_ = this.i18n('jemaosInstallerInstallREFIndHelperText');
        break;
      case MultiBootOptions.UEFI:
        this.multiBootOptionDesc_ = this.i18n('jemaosInstallerInstallUefiBootHelperText');
        break;
      case MultiBootOptions.NONE:
        this.multiBootOptionDesc_ = this.i18n('jemaosInstallerMultiBootConfigureByYourselfHelperText');
        break;
      default:
  ...
  computeInstallingTitle_(step, result) {
    if (step === InstallSteps.DONE) {
      if (result === InstallResults.SUCCESS) {
        return this.i18n('jemaosInstallerInstallSuccessTitle');
      } else if (result === InstallResults.FAILURE) {
        return this.i18n('jemaosInstallerInstallFailedTitle');
      } else if (result === InstallResults.ABORT) {
        return this.i18n('jemaosInstallerAbortTitle');
      }
    } else if (step === InstallSteps.ABORTING) {
      return this.i18n('jemaosInstallerAbortTitle');
    } else {
      return this.i18n('jemaosInstallerInstalling');
    }
  }
  ...
  async onAbortClicked_(e) {
    ...
    this.progressMessage_ = this.i18n('jemaosInstallerAbortingMessage');
    await this.installer_.forceCloseInstall();
    await this.safeUninstallDualboot_();
    this.setUIStep(InstallSteps.DONE);
    this.result_ = InstallResults.ABORT;
    this.progressMessage_ = this.i18n('jemaosInstallerAbortMessage');
  }
...
customElements.define(JemaOSInstallerScreen.is, JemaOSInstallerScreen);
