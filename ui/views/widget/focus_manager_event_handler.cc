...
void FocusManagerEventHandler::OnKeyEvent(ui::KeyEvent* event) {
  bool has_focused_view = widget_->GetFocusManager()->GetFocusedView() != nullptr;
#if BUILDFLAG(IS_CHROMEOS_ASH)
  bool special_key = event->IsControlDown() && event->key_code() == ui::VKEY_C && ash::features::IsJemaAssistantEnabled();
#else
