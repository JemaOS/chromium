...
class OobeWelcomeScreen extends OobeWelcomeScreenBase {
  ...
  setWindowJemaBackgroundColor_() {
    document.documentElement.setAttribute('jemaos-background-color', true);
  }

  unsetWindowJemaBackgroundColor_() {
    document.documentElement.removeAttribute('jemaos-background-color');
  }

  onWelcomeScreenUiStepChanged_(uiStep, isInWelcomeScreen) {
    // this.toggleJemaBackground_(uiStep, isInWelcomeScreen);
    this.mayPlayWelcomeSound_(uiStep, isInWelcomeScreen);
  }

  toggleJemaBackground_(uiStep, isInWelcomeScreen) {
    if (uiStep === WelcomeScreenState.GREETING && isInWelcomeScreen) {
      this.setWindowJemaBackgroundColor_();
    } else {
      this.unsetWindowJemaBackgroundColor_();
    }
  }
