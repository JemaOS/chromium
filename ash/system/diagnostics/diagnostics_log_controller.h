...
class ASH_EXPORT DiagnosticsLogController : SessionObserver {
 ...
 private:
  ...
  void GetJemaOsHwtunerInfo();
  void OnJemaOSHwtunerInfoReceived(absl::optional<jemaos::ash::ShellState> state);
  bool JemaosCreateSystemInfoTempDirectory();
  ...
  std::string jemaos_hwtuner_info_;
  std::string jemaos_system_info_;
  base::FilePath jemaos_system_info_temp_path_;
