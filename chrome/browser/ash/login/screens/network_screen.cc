...
void NetworkScreen::ShowImpl() {
  ...
  if (view_) {
    ...
    view_->ShowScreenWithData(std::move(data));
  }
}
...
void NetworkScreen::StopWaitingForConnection(const std::u16string& network_id) {
  ...
  // Auto-continue based on zero-touch flow is disabled in this build.
}
...
bool NetworkScreen::UpdateStatusIfConnectedToEthernet() {
  ...
  // Only skip the screen if WiFi is also connected; otherwise always show.
  if (!network_state_helper_->IsConnectedToWifi())
    return false;
...
void NetworkScreen::OnUiUpdateRequested(
  ...
  // Minimal handling to satisfy UiDelegate linkage.
  // Default to hiding Quick Start entry point unless explicitly enabled later.
  if (view_) {
    view_->SetQuickStartEntryPointVisibility(false);
  }
}
}  // namespace ash
