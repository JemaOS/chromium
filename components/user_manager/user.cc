...
bool User::TypeHasGaiaAccount(UserType user_type) {
  ...
  //---***JEMAOS BEGIN***---
         user_type == USER_TYPE_JEMA_ACCOUNT ||
         user_type == USER_TYPE_JEMA_CHILD ||
  //---***JEMAOS END***---
...
class JemaAccountUser: public RegularUser {
public:
  explicit JemaAccountUser(const AccountId& account_id, const UserType user_type);
  ~JemaAccountUser() override;
  // Overridden from User:
  UserType GetType() const override;
  void UpdateType(UserType user_type) override;
  bool CanSyncImage() const override;
...
bool User::IsJemaAccountUser() const {
  return GetType() == user_manager::USER_TYPE_JEMA_ACCOUNT || GetType() == user_manager::USER_TYPE_JEMA_CHILD;
}

bool User::IsJemaExtendAccountUser() const {
  return GetType() == user_manager::USER_TYPE_FLINT_ACCOUNT ||
         GetType() == user_manager::USER_TYPE_JEMA_ACCOUNT ||
         GetType() == user_manager::USER_TYPE_JEMA_CHILD;
}
//---***JEMAOS END***---

bool User::IsChild() const {
  return GetType() == USER_TYPE_CHILD || GetType() == USER_TYPE_JEMA_CHILD;
}
...
//---***JEMAOS BEGIN***---
  ...
  if (account_id.GetAccountType() == AccountType::JEMA_ACCOUNT)
    return new JemaAccountUser(account_id, user_type);
//---***JEMAOS END***---
...
UserType JemaAccountUser::GetType() const {
  return is_jema_child_ ? user_manager::USER_TYPE_JEMA_CHILD : user_manager::USER_TYPE_JEMA_ACCOUNT;
}

void JemaAccountUser::UpdateType(UserType user_type) {
  const UserType current_type = GetType();
  if ((user_type == user_manager::USER_TYPE_JEMA_CHILD ||
       user_type == user_manager::USER_TYPE_JEMA_ACCOUNT) &&
      (current_type == user_manager::USER_TYPE_JEMA_CHILD ||
       current_type == user_manager::USER_TYPE_JEMA_ACCOUNT)) {
    // We want all the other type changes to crash, that is why this check is
    // not at the top level.
    if (user_type == current_type)
      return;
    const bool old_is_child = is_jema_child_;
    is_jema_child_ = user_type == user_manager::USER_TYPE_JEMA_CHILD;

    LOG(WARNING) << "User type has changed: " << current_type
                 << " (is_child=" << old_is_child << ") => " << user_type
                 << " (is_child=" << is_jema_child_ << ")";
    UMAUserTypeChanged(is_jema_child_ ? UserTypeChangeHistogram::REGULAR_TO_CHILD
...
bool JemaAccountUser::CanSyncImage() const {
  return true;
}
//---***JEMAOS END***---

RegularUser::RegularUser(const AccountId& account_id, const UserType user_type)
    : User(account_id), is_child_(user_type == USER_TYPE_CHILD || user_type == USER_TYPE_JEMA_CHILD) {
  if (user_type != USER_TYPE_CHILD && user_type != USER_TYPE_REGULAR &&
//---***JEMAOS BEGIN***---
      user_type != USER_TYPE_FLINT_ACCOUNT && user_type != USER_TYPE_JEMA_ACCOUNT &&
      user_type != USER_TYPE_JEMA_CHILD &&
//---***JEMAOS END***---
...
JemaAccountUser::JemaAccountUser(const AccountId& account_id, const UserType user_type)
    : RegularUser(account_id, user_type),
      is_jema_child_(user_type == user_manager::USER_TYPE_JEMA_CHILD) {}

JemaAccountUser::~JemaAccountUser() {}
//---***JEMAOS END***---
