...
#define JEMAOS_API_KEY DUMMY_API_TOKEN
#endif
...
#define JEMAOS_CLIENT_ID_MAIN DUMMY_API_TOKEN
#endif

#if !defined(JEMAOS_CLIENT_SECRET_MAIN)
#define JEMAOS_CLIENT_SECRET_MAIN DUMMY_API_TOKEN
#endif
#endif
...
class APIKeyCache {
 public:
  APIKeyCache() {
    ...
    jemaos_api_key_ = CalculateKeyValue(
        JEMAOS_API_KEY, STRINGIZE_NO_EXPANSION(JEMAOS_API_KEY), nullptr,
        std::string(), environment.get(), command_line, gaia_config);
...
// A special non-stable key is at the moment defined only for Android Chrome.
...
#if BUILDFLAG(IS_OPENJEMA)
    std::string jemaos_default_client_id = CalculateKeyValue(
        JEMAOS_DEFAULT_CLIENT_ID,
        STRINGIZE_NO_EXPANSION(JEMAOS_DEFAULT_CLIENT_ID), nullptr,
        std::string(), environment.get(), command_line, gaia_config);
    std::string jemaos_default_client_secret = CalculateKeyValue(
        JEMAOS_DEFAULT_CLIENT_SECRET,
        STRINGIZE_NO_EXPANSION(JEMAOS_DEFAULT_CLIENT_SECRET), nullptr,
        std::string(), environment.get(), command_line, gaia_config);
#endif
...
#if BUILDFLAG(IS_OPENJEMA)
    client_ids_[CLIENT_JEMAOS_MAIN] = CalculateKeyValue(
        JEMAOS_CLIENT_ID_MAIN, STRINGIZE_NO_EXPANSION(JEMAOS_CLIENT_ID_MAIN),
        ::switches::kOAuth2JemaOsClientID, jemaos_default_client_id, environment.get(),
        command_line, gaia_config);
    client_secrets_[CLIENT_JEMAOS_MAIN] = CalculateKeyValue(
        JEMAOS_CLIENT_SECRET_MAIN,
        STRINGIZE_NO_EXPANSION(JEMAOS_CLIENT_SECRET_MAIN),
        ::switches::kOAuth2JemaOsClientSecret, jemaos_default_client_secret,
        environment.get(), command_line, gaia_config);
#endif
  ...
  std::string jemaos_api_key() const { return jemaos_api_key_; }
...
bool HasJemaOSAPIKeyConfigured() {
  return GetJemaOSAPIKey() != DUMMY_API_TOKEN;
}
...
std::string GetJemaOSAPIKey() {
  return g_api_key_cache.Get().jemaos_api_key();
}
...
bool HasJemaOAuthClientConfigured() {
  OAuth2Client client = CLIENT_JEMAOS_MAIN;
...
#if BUILDFLAG(IS_OPENJEMA)
    if (client_id == CLIENT_JEMAOS_MAIN) {
      continue;
    }
#endif
