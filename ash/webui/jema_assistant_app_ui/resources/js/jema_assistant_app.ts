// Copyright 2025 jema technology
...
import {getTemplate} from './jema_assistant_app.html.js';
...
const JEMA_ASSISTANT_APP_WEBVIEW_PARTITION = 'persist:jemaosAssistant';

const JemaAssistantAppElementBase = PolymerElement;

export class JemaAssistantAppElement extends JemaAssistantAppElementBase {
  static get is() {
    return 'jema-assistant-app' as const;
  }
  ...
  constructor() {
    ...
    if(!loadTimeData.getBoolean('isJemaOSAssistantEnabled')) {
      window.open('chrome://os-settings/osJemaAssistant');
      window.close();
    }
    if(!loadTimeData.getBoolean('isJemaOSAssistantEnabled')) {
      window.open('chrome://os-settings/osJemaAssistant');
  ...
  override connectedCallback(): void {
    ...
    this.webview_.partition = JEMA_ASSISTANT_APP_WEBVIEW_PARTITION;
  ...
  getUrl_() {
    let url = loadTimeData.getString('jemaosAssistantUrl');
  ...
  onNewWindow_(e: Event): void {
    ...
    if (partition !== JEMA_ASSISTANT_APP_WEBVIEW_PARTITION) {
      return;
    }
    e.preventDefault();
...
declare global {
  interface HTMLElementTagNameMap {
    [JemaAssistantAppElement.is]: JemaAssistantAppElement;
  }
}
