...
FeaturePodButton* RotateScreenFeaturePodController::CreateButton() {
  ...
  button_->SetVectorIcon(kUnifiedMenuRotateScreenIcon);
  const auto label_text = l10n_util::GetStringUTF16(
...
void RotateScreenFeaturePodController::UpdateButton() {
  ...
  if (is_qs_revamp_enabled) {
    ...
    button_->SetVisible(is_in_tablet_physical_state && visible);
  }
}
