...
void SyncServiceImpl::StartSyncingWithServer() {
  ...
  // ---***JEMAOS BEGIN***---
  if(!jemaos::switches::IsJemaAccountEnabled()) return;
  ...
  if (engine_ && !family_link_timer_->IsRunning()) {
    int interval = jemaos::switches::GetJemaOSSupervisedUserSettingsSyncInterval();
    VLOG(1) << "check for jemaos supervised user settings sync data every " << interval << " seconds";
