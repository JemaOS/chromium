...
//---***JEMAOS BEGIN***---
void NetworkStateHandler::ShellStateCallback(base::OnceClosure callback, absl::optional<ShellState> state) {
...
void NetworkStateHandler::InvokeExecuteReloadWifiDrv(base::OnceClosure callback) {
    ...
    JemaOSShellClient::Get()
      ->SyncExec(jemaos::constants::kJemaOSReloadWifiCmd,
        base::BindOnce(&NetworkStateHandler::ShellStateCallback, AsWeakPtr(), std::move(callback)));
}
//---***JEMAOS END***---
...
//---***JEMAOS BEGIN***---
  need_reload_wifidrv = jemaos::switches::NeedResetWifiDriver();
//---***JEMAOS END***---
}
...
void NetworkStateHandler::PerformSetTechnologyEnabled(
  ...
  if (need_reload_wifidrv && enabled && technology == shill::kTypeWifi
    ...
    InvokeExecuteReloadWifiDrv(base::BindRepeating(
          ...
          shill_property_handler_->AsWeakPtr(), technology, enabled,
