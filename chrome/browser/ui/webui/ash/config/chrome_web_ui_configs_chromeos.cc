...
void RegisterAshChromeWebUIConfigs() {
  ...
  if (base::FeatureList::IsEnabled(ash::features::kJemaAssistant)) {
    map.AddWebUIConfig(
        MakeComponentConfigWithDelegate<JemaAssistantAppUIConfig, JemaAssistantAppUI,
                                        ChromeJemaAssistantAppUIDelegate>());
  }
#if !defined(OFFICIAL_BUILD)
