...
import {getTemplate} from './jemaos_remoting.html.js';
...
class RemoteHelperClient {
  ...
  async getRemoteHelperPort() {
    const port = await this.shellClient_.SafeExecForResult('cat /run/jemaos_remote_helper/port');
    return port;
  }
...
const JemaSettingsRemotingPageElementBase =
  RouteObserverMixin(I18nMixin(PolymerElement));

class JemaSettingsRemotingPageElement extends JemaSettingsRemotingPageElementBase {
  static get is() {
    return 'os-settings-jemaos-remoting' as const;
  }
  ...
  constructor() {
    super();
    console.log('os-settings-jemaos-remoting ready');
  ...
  override connectedCallback() {
    ...
    console.log('os-settings-jemaos-remoting attached');
  }
...
// </if>
    chrome.nativeWindows.create(appId, () => {
      chrome.nativeWindows.getAll(list => {
        ...
        if (!created) {
          const url = loadTimeData.getString('jemaOSRdpUrl');
  ...
  getRemoteHelperMessage_() {
    if (this.remoteHelperState_ === RemoteHelperClient.RemoteHelperStates.REQUIRE_RESTART) {
      return this.i18n('jemaosSettingsRemoteHelperRequireRestartMessage');
    }
    return '';
  }
  ...
  getPaperTooltipMessage_() {
    if (this.remoteHelperState_ === RemoteHelperClient.RemoteHelperStates.ENABLED) {
      return this.i18n('jemaosSettingsRemoteHelperEnabledMessage');
    }
    return this.i18n('jemaosSettingsRemoteHelperDesc');
  }
...
customElements.define(
    JemaSettingsRemotingPageElement.is, JemaSettingsRemotingPageElement);
