...
import {getTemplate} from './jemaos_installer.html.js';
...
"blockdevices": [
  ...
  DualbootInstall({ partPath, efiPath, installrEFI = true, installBoot = false }) {
    ...
    const process = [
      ...
      {
        // <if expr="openjema">
        command: JemaOSInstaller.UtilWrapperCMD('safe_create_entry', ['/EFI/openjema/bootx64.efi', 'openJema_Dualboot_Loader', efiPath]),
        // </if>
...
const JemaOSInstallerScreenElementBase = OobeDialogHostMixin(LoginScreenMixin(MultiStepMixin(OobeI18nMixin(PolymerElement))));
