...
namespace {
  ...
  constexpr char kJemaOSLookingGlassPath[] = "/api/collect";
...
void StatisticsCollector::CollectUserInfo() {
  ...
  if (account_id.HasAccountIdKey()) {
    ...
    base::Base64Encode(account_id_hash, &statistics_.profile_account_id);
  }
...
bool StatisticsCollector::EncryptData(
  ...
  base::Base64Encode(std::string(result.begin(), result.end()), output);
  if (output->empty()) {
