...
bool EulaScreen::MaybeSkip(WizardContext& context) {
  ...
  if (!context.is_branded_build && !jemaos::switches::IsJemaCustomEnabled()) {
...
void EulaScreen::ShowImpl() {
  ...
  if (WizardController::IsZeroTouchHandsOffOobeFlow() || WizardController::IsJemaZeroTouchEnrollFlow()) {
