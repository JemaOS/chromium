...
class JemaSettingsAccountPageElement extends JemaSettingsAccountPageElementBase {
  static get is() {
    return 'os-settings-jemaos-account';
  }
  ...
  static get properties() {
    return {
      ...
      isProfileActionable_: {
        type: Boolean,
        value() {
          return loadTimeData.getBoolean('isJemaProfile') && !loadTimeData.getBoolean('isJemaLocalAccount');
        }
      },
  ...
  getOfflineAutoSigninMessage_() {
    const defaultMessage = this.i18n('enableAutoSigninForJemaLocalAccountHelpMessage');
    if (!this.isJemaLocalAccount_) {
      return this.i18n('unableToSetAutoSigninForJemaNonLocalAccount');
    }
    ...
    if (this.isOfflineAutoSigninEnabled_) {
      if (this.isOfflineAutoSigninEnabledForCurrentUser_) {
        ...
        return this.i18n('autoSigninForJemaLocalAccountOtherUserAlreadyEnabled');
      }
    } else {
      if (this.systemSaltObtained_ && this.authFactorHasPassword_) {
        return defaultMessage;
      } else {
        return this.i18n('unableToSetAutoSigninForJemaLocalAccount');
      }
    }
  }
  ...
  shouldShowSetPasswordLink_() {
    return this.isJemaLocalAccount_ && !this.authFactorHasPassword_;
  }
