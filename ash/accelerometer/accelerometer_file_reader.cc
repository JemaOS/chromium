...
// ---***JEMAOS BEGIN***---
const base::FilePath::CharType kJemaOSScaleFileName[] = "in_accel_scale";
...
const struct DataPattern KJemaOSDataPattern[3] = {
    {6, {0, 1, 2}},
    {12, {0, 1, 3}},
    {12, {0, 1, 2}},
  };
...
void AccelerometerFileReader::PrepareAndInitialize() {
  ...
  int jemaos_initialize_timeout = jemaos::switches::GetJemaOSAccelerometerInitializeTimeoutInSeconds();
  if (jemaos_initialize_timeout > 0) {
    initialization_timeout_ = base::TimeTicks::Now() + base::Seconds(jemaos_initialize_timeout);
  } else {
...
AccelerometerFileReader::InitializeInternal() {
  ...
  for (base::FilePath name = symlink_dir.Next(); !name.empty();
    ...
    if (legacy_cross_accel) {
      //---***JEMAOS BEGIN***---
      if (jemaos::switches::IsJemaOSAccelerometer()) {
        if (!InitializeJemaOSAccelerometer(iio_path, name)) {
          result.initialization_state = State::FAILED;
          return result;
        }
      } else {
...
// ---***JEMAOS BEGIN***---
 if (!jemaos::switches::GetAccelDataPattern()) {
// ---***JEMAOS END***---
  for (int i = 0; i < ACCELEROMETER_SOURCE_COUNT; ++i) {
...
// ---***JEMAOS END***---
  ...
  //---***JEMAOS BEGIN***---
  int delayMS = jemaos::switches::GetJemaOSAccelerometerReadIntervalInMS();
  if (delayMS)
...
//---***JEMAOS BEGIN***---
bool AccelerometerFileReader::InitializeJemaOSAccelerometer(
  const base::FilePath& iio_path,
    const base::FilePath& name){
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  double scale;
  size_t pattern = jemaos::switches::GetAccelDataPattern();
  if (!ReadFileToDouble(iio_path.Append(kJemaOSScaleFileName), &scale))
          return false;
  VLOG(1) << __func__ << " Scale:" << scale;
  int config = jemaos::switches::GetAccelConfig();
  for (size_t i = 0; i < kNumberOfAxes; ++i) {
    std::string accelerometer_index_path = base::StringPrintf(
        kAccelerometerScanIndexPathFormatString, kJemaOSConfig[config][i]);
    ...
    if (pattern >= 0 && pattern < std::size(KJemaOSDataPattern))
    configuration_.index[ACCELEROMETER_SOURCE_SCREEN][i] =
      KJemaOSDataPattern[pattern].data_index[configuration_.index[ACCELEROMETER_SOURCE_SCREEN][i]];
  ...
  configuration_.right_move[ACCELEROMETER_SOURCE_SCREEN] = jemaos::switches::GetAccelRightMoveBits();
  if (jemaos::switches::IsAccelRevertX())
      configuration_.revert[ACCELEROMETER_SOURCE_SCREEN][0] = -1;
  if (jemaos::switches::IsAccelRevertY())
      configuration_.revert[ACCELEROMETER_SOURCE_SCREEN][1] = -1;
  if (jemaos::switches::IsAccelRevertZ())
      configuration_.revert[ACCELEROMETER_SOURCE_SCREEN][2] = -1;
  configuration_.count++;
  configuration_.swap_bytes = jemaos::switches::JemaOSAccelerometerSwapBytes();

  ReadingData reading_data;
  reading_data.path =
      base::FilePath(kAccelerometerDevicePath).Append(name.BaseName());
  if (pattern >= 0 && pattern < std::size(KJemaOSDataPattern)){
    reading_data.data_size = KJemaOSDataPattern[pattern].data_size;
