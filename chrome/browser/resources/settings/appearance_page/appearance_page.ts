...
export class SettingsAppearancePageElement extends
  ...
  private onThemeClick_() {
    if (this.toolbarPinningEnabled_) {
      ...
      if (!this.storeAppExists_) {
        console.error('[appearance page] store app does not exists');
        return;
      }
      const jemaosStoreBaseUrl = loadTimeData.getString('jemaosStoreBaseUrl');
      const themeHomeUrl_ = `${jemaosStoreBaseUrl}/?init=themes`;
      const url = this.themeUrl_  || themeHomeUrl_;
  ...
  private themeChanged_(themeId: string) {
    ...
    if (themeId.length > 0 && themeId !== AUTOGENERATED_THEME_ID &&
      ...
      //---***JEMAOS BEGIN***---
      const jemaosStoreBaseUrl = loadTimeData.getString('jemaosStoreBaseUrl');
      this.themeUrl_ = loadTimeData.getBoolean('isJemaProfile') ?
        `${jemaosStoreBaseUrl}/?init=themes&appid=${themeId}` :
  ...
  private showThemesURL_(visibility: boolean, storeAppExists: boolean): boolean {
    return (visibility !== false) && (!loadTimeData.getBoolean('isJemaProfile') || storeAppExists);
  }

  private showThemesHr_(visibility: boolean, storeAppExists: boolean, nextIsVisible: boolean): boolean {
    return this.showThemesURL_(visibility, storeAppExists) && nextIsVisible;
  }
}
