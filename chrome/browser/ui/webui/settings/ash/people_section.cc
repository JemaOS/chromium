...
const std::vector<SearchConcept>& GetNonJemaAccountPeopleSearchConcepts() {
  static const base::NoDestructor<std::vector<SearchConcept>> tags([] {
...
PeopleSection::PeopleSection(Profile* profile,
  ...
  if (profile && profile->IsJemaProfile()) {
    updater.RemoveSearchTags(GetNonJemaAccountPeopleSearchConcepts());
  }
...
void PeopleSection::AddLoadTimeData(content::WebUIDataSource* html_source) {
  ...
  html_source->AddBoolean("isAccountManagerEnabled",
                          account_manager_facade_ != nullptr && !profile()->IsJemaProfile());
  html_source->AddBoolean(
